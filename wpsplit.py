...
#
# Original script by https://wordpress.org/support/profile/eerac
# https://wordpress.org/support/topic/wxr-file-splitter
#
# Modified by Lon Koenig to use a regular expression for detecting the <item> delimiter
import os
import sys
import math
import re
...
if len(sys.argv) < 2 :
	...
	print 'Syntax: wpsplit.py {exportfile.xml} {number of files to split into}'
	sys.exit(0)
...
line_delimiter = '\s*<item>\n'

delimiter_count = 0
for line in lines :
	#if line == line_delimiter :
	if  re.match(line_delimiter, line) :
		delimiter_count += 1
...
for line in lines :
	...
	if  re.match(line_delimiter, line) : delimiter_count += 1
